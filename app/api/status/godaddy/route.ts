...
export async function GET() {
  try {
    const res = await fetch('https://status.godaddy.com/api/v2/summary.json', {
    ...
    const activeIncidents: Incident[] = (data.incidents || [])
      ...
      .map((incident: any) => ({
        ...
        url: incident.shortlink || `https://status.godaddy.com/incidents/${incident.id}`,
      }));
    ...
    return Response.json({
      ...
      incidents: [],
      lastUpdated: new Date().toISOString(),
      error: 'Unable to fetch GoDaddy status',
    });
  }
}
