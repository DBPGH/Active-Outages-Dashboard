...
export async function GET() {
  try {
    const res = await fetch('https://status.getthread.com/api/v2/summary.json', {
    ...
    const activeIncidents: Incident[] = (data.incidents || [])
      ...
      .map((incident: any) => ({
        ...
        url: incident.shortlink || `https://status.getthread.com/incidents/${incident.id}`,
      }));
    ...
    return Response.json({
      ...
      incidents: [],
      lastUpdated: new Date().toISOString(),
      error: 'Unable to fetch Thread status',
    });
  }
}
